...
describe('POST /api/pulls/:o/:r/:n/ai/ask', () => {
  ...
  it('builds a structured prompt with title, author, diff, and draft, and returns the response', async () => {
    ...
    // tokensUsed isn't parsed for the Claude backend (only Codex emitted it).
    expect(res.json()).toEqual({ response: 'Looks fine. One concern: ...', truncatedDiff: false, tokensUsed: undefined });
    ...
    // Confirm Sonnet + read-only allowlist are on the CLI options.
    const opts = mockedClaude.mock.calls[0][1] as { model?: string; allowedTools?: string[]; permissionMode?: string };
