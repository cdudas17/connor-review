...
export function useTrackedPRs() {
  ...
  const add = useCallback((pr: Omit<TrackedPR, 'status' | 'addedAt' | 'ghStatus' | 'ciStatus' | 'createdAt'> & { ghStatus?: TrackedPR['ghStatus']; ciStatus?: TrackedPR['ciStatus']; createdAt?: TrackedPR['createdAt'] }) => {
    setPrs((cur) => (cur.some((p) => same(p, pr))
      ? cur
      : [...cur, { ghStatus: null, ciStatus: null, createdAt: null, ...pr, status: 'untouched', addedAt: Date.now() }]));
  }, []);
  ...
  const update = useCallback((id: Identity, patch: Partial<Pick<TrackedPR, 'title' | 'authorLogin' | 'ghStatus' | 'ciStatus' | 'createdAt'>>) => {
